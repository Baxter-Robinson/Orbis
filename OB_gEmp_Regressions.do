...
*XXX Labels might be wrong: Public vs. Private
preserve

	keep IDNum nEmployees Public EmpGrowth_h NACE_Rev_2_Core_code_4_digits Year US_SIC_Core_code_3_digits Industry_4digit
	...
	gEmp_ft = const + alpha * lnEmp + beta * lnEmp*1{Public} + Sector FE + Time FE + error
	Where lnEmp is log of employment. 
	*/
	destring, replace 
	gen lnEmp = log(nEmployees)
	*gen lnEmp = asinh(nEmployees)
	gen lnEmp_Priv = lnEmp*Public 

	xtset IDNum Year

	* Using the NACE Rev.2 4-digit codes

	reg EmpGrowth_h lnEmp Public, robust
	eststo m1
	...
	reghdfe EmpGrowth_h lnEmp Public, absorb( Year) vce(robust)
	eststo m1a
	estadd local Sector "No"
	...
	reghdfe EmpGrowth_h lnEmp Public, absorb(Industry_4digit Year) vce(robust)
	eststo m1b
	estadd local Sector "Yes"
	estadd local Year "Yes"
	estadd local Robust "Yes"

	esttab m1 m1a m1b  using "Output/$CountryID/OB_Emp_growth_regs_NACE.tex", se legend mtitles("NACE Rev2 4-digit Sectors" "NACE Rev2 4-digit Sectors" "NACE Rev2 4-digit Sectors") title("Employment growth") s(N Sector Year Robust, label( "N" "Sector Fixed Effect" "Year Fixed Effect" "Robust S.E."))  varlabels(_cons "Constant" lnEmp "(Log of) No. employees" Public "Public Firm") nonumbers keep( _cons lnEmp Public) replace fragment
	...
	* Using the SIC 3-digit codes
	...
	reghdfe EmpGrowth_h lnEmp Public, absorb(US_SIC_Core_code_3_digits Year) vce(robust)
	eststo m2b
	estadd local Sector "Yes"
	estadd local Year "Yes"
	estadd local Robust "Yes"
...
preserve
	...
	/* 
	gEmp_ft = const + alpha * SizeCat + beta * SizeCat*1{Public} + Sector FE + Time FE + error

	Where SizeCat is a set of BINARY variables over for the values {1,2-10,11-50,51-200,201-999,1000+} 
	*/


	keep IDNum nEmployees Public EmpGrowth_h NACE_Rev_2_Core_code_4_digits Year US_SIC_Core_code_3_digits Industry_4digit SizeCategoryAR
	...
	* Using the NACE Rev.2 4-digit codes
	...
	esttab m1 m1a m1b using "Output/$CountryID/OB_Emp_growth_regs_SizeCats_NACE.tex", se legend mtitles("NACE Rev2 4-digit Sectors" "NACE Rev2 4-digit Sectors" "NACE Rev2 4-digit Sectors") title("Employment growth") s(N Sector Year Robust, label( "N" "Sector Fixed Effect" "Year Fixed Effect" "Robust S.E."))  varlabels(_cons "Constant" SizeGroup2 "2-5 Employees" SizeGroup3 "6-10 Employees" SizeGroup4 "11-50 Employees" SizeGroup5 "51-100 Employees" SizeGroup6 "101-1000 Employees" SizeGroup7 "More than 1000 Employees"  SizeGroup1_Priv "1 Employee x Public" SizeGroup2_Priv "2-5 Employees x Public" SizeGroup3_Priv "6-10 Employees x Public" SizeGroup4_Priv "11-50 Employees x Public" SizeGroup5_Priv "51-100 Employees x Public" SizeGroup6_Priv "101-1000 Employees x Public" SizeGroup7_Priv "More than 1000 Employees x Public") nonumbers keep( _cons SizeGroup2 SizeGroup3 SizeGroup4 SizeGroup5 SizeGroup6 SizeGroup7 SizeGroup1_Priv SizeGroup2_Priv SizeGroup3_Priv SizeGroup4_Priv SizeGroup5_Priv SizeGroup6_Priv SizeGroup7_Priv) replace fragment
