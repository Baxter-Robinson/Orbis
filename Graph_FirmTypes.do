...
** For each Firm Type
collapse (mean) nShareholders SalesPerEmployee nEmployees Age (count) nFirms=nShareholders, by(FirmType)


replace nFirms=nFirms/1000

twoway (bar nFirms FirmType) , xtitle("Number of Shareholders") ytitle("Number of Firms")  graphregion(color(white)) xlabel(  1 "1" 2 "2" 3 ">2"  6 "Public" )
...
drop Closing_date Export_revenue Stock Market_capitalisation_mil Age
...
** For each Firm Type
collapse (mean) EmpGrowth SalesGrowth  SalesPerEmpGrowth ///
